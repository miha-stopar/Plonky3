...
const SHADER_MSL: &str = include_str!("../shaders/koalabear_whir.metal");
...
pub struct MetalInner {
    ...
    dif_r32_oop_ps: ComputePipelineState,
    ...
    keccak_hash_leaves_ps: ComputePipelineState,
    keccak_hash_and_compress_ps: ComputePipelineState,
    keccak_hash4_compress3_ps: ComputePipelineState,
    keccak_compress_ps: ComputePipelineState,
    keccak_compress_two_levels_ps: ComputePipelineState,
    combine_select_ps: ComputePipelineState,
    transpose_pad_ps: ComputePipelineState,
    transpose_pad_packed_ps: ComputePipelineState,
...
pub struct GpuMerkleResultU64 {
    ...
    pub arity_schedule: Vec<usize>,
    gpu_backed: Option<(usize, Box<dyn std::any::Any + Send + Sync>)>,
    /// When set, leaf matrix memory is zero-copy from this DFT output buffer.
    leaf_keepalive: Option<Buffer>,
}
...
impl GpuMerkleResultU64 {
    pub fn into_tree<M: p3_matrix::Matrix<KoalaBear>>(
        ...
        assert!(
            ...
            "use into_tree_zc for zero-copy leaves"
        );
        let tree = match self.gpu_backed {
            Some((count, keepalive)) => unsafe {
                p3_merkle_tree::MerkleTree::from_parts_gpu_backed(
    ...
    pub fn into_tree_zc<M: p3_matrix::Matrix<KoalaBear> + Send + Sync + 'static>(
        ...
        let tree = match (self.gpu_backed, self.leaf_keepalive) {
            (Some((count, merkle_ka)), Some(dft_buf)) => {
                let combined: Box<dyn std::any::Any + Send + Sync> =
                    match merkle_ka.downcast::<MerkleBufferKeepalive>() {
                        Ok(mk) => Box::new(DualBufferKeepalive { _merkle: mk.0, _dft: dft_buf }),
                        Err(mk) => {
                            let pair: (Box<dyn std::any::Any + Send + Sync>, Buffer) = (mk, dft_buf);
                            Box::new(pair)
                ...
                let leaf_forget_fn: Box<dyn FnOnce(&mut Vec<M>) + Send + Sync> =
                    Box::new(|leaves: &mut Vec<M>| {
                        let taken = core::mem::take(leaves);
                        core::mem::forget(taken);
                    });
                unsafe {
...
/// When `false` / `0`, Keccak Merkle uses separate leaf-hash + compress dispatches only.
/// Default: fused `hash_and_compress` / `hash4_compress3` paths enabled.
fn keccak_merkle_fused_enabled() -> bool {
    std::env::var("WHIR_KECCAK_MERKLE_FUSED")
        .map(|v| v != "0" && !v.eq_ignore_ascii_case("false"))
        .unwrap_or(true)
}

/// Max threads per threadgroup for Keccak Merkle kernels (clamped against pipeline max).
/// Default 1024: lower aggregate full-proving Keccak time vs 256 on the `protocol_compare` grid
/// (some small shapes regress; override with `WHIR_KECCAK_MERKLE_TG_CAP`).
fn keccak_merkle_tg_cap() -> u64 {
    std::env::var("WHIR_KECCAK_MERKLE_TG_CAP")
        .ok()
...
/// Fuse two Merkle compress dispatches into one (`keccak_merkle_compress_two_levels`).
fn keccak_merkle_compress2_enabled() -> bool {
    std::env::var("WHIR_KECCAK_MERKLE_COMPRESS2")
        .map(|v| v != "0" && !v.eq_ignore_ascii_case("false"))
        .unwrap_or(true)
}

/// Max threads per threadgroup for **Keccak Merkle compress** kernels only
/// (`keccak_merkle_compress`, `keccak_merkle_compress_two_levels`).
///
/// Default: `min(WHIR_KECCAK_MERKLE_TG_CAP, 256)` so compress dispatches match the
/// Poseidon `encode_merkle_tree` heuristic (`.min(256)` on compress), which can
/// improve occupancy when pair counts are modest. Override with
/// `WHIR_KECCAK_MERKLE_COMPRESS_TG_CAP` (e.g. `1024` to mimic pre-change behavior).
fn keccak_merkle_compress_tg_dispatch_cap() -> u64 {
    std::env::var("WHIR_KECCAK_MERKLE_COMPRESS_TG_CAP")
        .ok()
        .and_then(|v| v.parse().ok())
        .unwrap_or_else(|| keccak_merkle_tg_cap().min(256))
        .max(1)
}
...
impl MetalKoalaBearDft {
    ...
    fn new_with_params(cpu: Radix2DFTSmallBatch<KoalaBear>, gpu_min_log_n: u32, poseidon2_seed: u64) -> Self {
        ...
        if std::env::var("WHIR_METAL_FAST_MATH")
            .map(|v| v != "0" && !v.eq_ignore_ascii_case("false"))
            .unwrap_or(false)
        {
        ...
        let keccak_hash_leaves_ps = make_ps("keccak_hash_leaves");
        let keccak_hash_and_compress_ps = make_ps("keccak_hash_and_compress");
        ...
        Self { inner: Arc::new(MetalInner {
            ...
            keccak_hash_leaves_ps,
            keccak_hash_and_compress_ps,
            keccak_hash4_compress3_ps,
            keccak_compress_ps,
            keccak_compress_two_levels_ps,
            combine_select_ps,
            transpose_pad_ps,
            transpose_pad_packed_ps,
            poseidon2_rc_buf,
            buf_copy_ps,
    ...
    /// Encode Keccak-based Merkle tree dispatches into an existing encoder.
    /// Keccak digest layout is `[u64; 4]` (32 bytes).
    fn encode_keccak_merkle_tree(
        &self,
        enc: &ComputeCommandEncoderRef,
        data_buf: &metal::BufferRef,
        num_leaves: u32,
        ...
        let digest_elems = 4u64;
        let elem_bytes = size_of::<u64>() as u64;
        ...
        let fused = keccak_merkle_fused_enabled();
        let tg_cap = keccak_merkle_tg_cap();
        let compress_tg_cap = keccak_merkle_compress_tg_dispatch_cap();

        let start_layer: usize = if fused && num_leaves >= 4 && layer_counts.len() >= 3 {
            let quads = num_leaves / 4;
            let max_tg = self.keccak_hash4_compress3_ps.max_total_threads_per_threadgroup() as u64;
            enc.set_compute_pipeline_state(&self.keccak_hash4_compress3_ps);
            enc.set_buffer(0, Some(data_buf), 0);
            enc.set_buffer(1, Some(&buf), offsets[0]);
            enc.set_buffer(2, Some(&buf), offsets[1]);
            enc.set_buffer(3, Some(&buf), offsets[2]);
            ...
            enc.dispatch_threads(
                MTLSize { width: quads as u64, height: 1, depth: 1 },
                MTLSize { width: max_tg.min(tg_cap), height: 1, depth: 1 },
            );
            3
        } else if fused && num_leaves >= 2 && layer_counts.len() >= 2 {
            let pairs = num_leaves / 2;
            let max_tg = self.keccak_hash_and_compress_ps.max_total_threads_per_threadgroup() as u64;
            enc.set_compute_pipeline_state(&self.keccak_hash_and_compress_ps);
            enc.set_buffer(0, Some(data_buf), 0);
            enc.set_buffer(1, Some(&buf), offsets[0]);
            enc.set_buffer(2, Some(&buf), offsets[1]);
            set_u32(enc, 3, pairs);
            ...
            let max_tg_leaves =
                self.keccak_hash_leaves_ps.max_total_threads_per_threadgroup() as u64;
            enc.set_compute_pipeline_state(&self.keccak_hash_leaves_ps);
            enc.set_buffer(0, Some(data_buf), 0);
            enc.set_buffer(1, Some(&buf), offsets[0]);
            set_u32(enc, 2, num_leaves);
            set_u32(enc, 3, leaf_width);
            enc.dispatch_threads(
                MTLSize { width: num_leaves as u64, height: 1, depth: 1 },
                MTLSize { width: max_tg_leaves.min(tg_cap), height: 1, depth: 1 },
            );
            1
        };

        let max_tg_compress = self.keccak_compress_ps.max_total_threads_per_threadgroup() as u64;
        let max_tg_c2 = self.keccak_compress_two_levels_ps.max_total_threads_per_threadgroup() as u64;
        let mut current_count = num_leaves >> (start_layer.saturating_sub(1)).min(31);
        let mut layer_idx = start_layer;
        let compress2 = fused && keccak_merkle_compress2_enabled();
        while layer_idx < layer_counts.len() {
            if compress2
                && current_count >= 4
                && current_count % 4 == 0
                && layer_idx + 1 < layer_counts.len()
            {
                let quads = current_count / 4;
                enc.set_compute_pipeline_state(&self.keccak_compress_two_levels_ps);
                enc.set_buffer(0, Some(&buf), offsets[layer_idx - 1]);
                enc.set_buffer(1, Some(&buf), offsets[layer_idx]);
                enc.set_buffer(2, Some(&buf), offsets[layer_idx + 1]);
                set_u32(enc, 3, quads);
                enc.dispatch_threads(
                    MTLSize { width: quads as u64, height: 1, depth: 1 },
                    MTLSize {
                        width: max_tg_c2.min(compress_tg_cap),
                        height: 1,
                        depth: 1,
                    },
                );
                ...
                let pairs = current_count / 2;
                enc.set_compute_pipeline_state(&self.keccak_compress_ps);
                enc.set_buffer(0, Some(&buf), offsets[layer_idx - 1]);
                enc.set_buffer(1, Some(&buf), offsets[layer_idx]);
                set_u32(enc, 2, pairs);
                enc.dispatch_threads(
                    MTLSize { width: pairs as u64, height: 1, depth: 1 },
                    MTLSize {
                        width: max_tg_compress.min(compress_tg_cap),
                        height: 1,
                        depth: 1,
                    },
                );
    ...
    fn read_keccak_merkle_layers_zero_copy(layers: MerkleLayers) -> GpuMerkleResultU64 {
        let base_ptr = layers.buf.contents() as *const u8;
        let num_layers = layers.num_digests.len();
        let arity_schedule = vec![2usize; num_layers.saturating_sub(1)];

        let root_offset = *layers.offsets.last().expect("non-empty merkle layers");
        let root_digest = unsafe { *(base_ptr.add(root_offset as usize) as *const [u64; 4]) };
        let cap = p3_symmetric::MerkleCap::new(vec![root_digest]);

        let digest_layers: Vec<Vec<[u64; 4]>> = layers
            .num_digests
            .iter()
            .zip(layers.offsets.iter())
            .map(|(&count, &offset)| {
                let n = count as usize;
                let ptr = unsafe { base_ptr.add(offset as usize) as *mut [u64; 4] };
                unsafe { Vec::from_raw_parts(ptr, n, n) }
            })
            .collect();
    ...
    /// Fused DFT + Keccak Merkle: same single-buffer fusion as [`Self::gpu_dft_and_merkle`],
    ...
    pub fn gpu_dft_and_keccak_merkle(
        ...
        let gpu_result: Option<(bool, MerkleLayers)> = autoreleasepool(|| {
            ...
            if let Some(ref zc_buf) = zc_input {
                if use_four_step {
                    ...
                    self.encode_four_step_ntt_oop(&enc, zc_buf, zc_buf, &temp, &tw,
                        log_n, height as u32, width as u32);
                    let merkle = self.encode_keccak_merkle_tree(
                        &enc, zc_buf, height as u32, width as u32,
                    );
                    enc.end_encoding();
                    cmd.commit();
        ...
        gpu_result.map(|(_, merkle)| {
            Self::read_keccak_merkle_layers_zero_copy(merkle)
        })
    }
    ...
    /// Like `gpu_transpose_dft_and_merkle` but input is in NEON packed (SoA) layout.
    ...
    fn gpu_transpose_dft_and_merkle_inner(
        ...
        let src_bytes = (data_len * size_of::<u32>()) as u64;
        let opts = MTLResourceOptions::CPUCacheModeDefaultCache
            | MTLResourceOptions::StorageModeShared;
        let src_buf = Self::make_source_buffer(&self.device, data_ptr, src_bytes, opts);
        ...
        let gpu_result: Option<(MerkleLayers, Vec<KoalaBear>, Option<bool>)> = autoreleasepool(|| {
            let transpose_ps = if packed_input {
                ...
                &self.transpose_pad_ps
            };
            let encode_transpose = |enc: &ComputeCommandEncoderRef| {
                let set_u32 = |enc: &ComputeCommandEncoderRef, index: u64, val: u32| {
                    enc.set_bytes(index, size_of::<u32>() as u64, (&val as *const u32).cast());
                };
                enc.set_compute_pipeline_state(transpose_ps);
                enc.set_buffer(0, Some(&src_buf), 0);
                ...
                set_u32(enc, 4, out_height as u32);
                set_u32(enc, 5, elem_size as u32);
                let tg = transpose_ps.max_total_threads_per_threadgroup() as u64;
                let tg_w = (in_rows as u64).min(16);
                let tg_h = (tg / tg_w).min(64);
                enc.dispatch_threads(
    ...
    fn gpu_transpose_dft_and_keccak_merkle_inner(
        ...
        let gpu_result: Option<(MerkleLayers, Vec<KoalaBear>, Option<bool>)> = autoreleasepool(|| {
            ...
            if profiling {
                ...
                eprintln!(
                    "[GPU_PROFILE] transpose_dft_keccak_merkle: log_n={log_n} width={out_width} \
                     | DFT {dft_ms:.1}ms | Merkle {merkle_ms:.1}ms | copy {copy_ms:.1}ms \
                     | total {:.1}ms",
                    dft_ms + merkle_ms + copy_ms,
                );
        ...
        gpu_result.map(|(merkle, out_values, zc_buf_is_dif)| {
            match zc_buf_is_dif {
                Some(true) => {
                    Self::release_buf(&self.temp_buf_cache, natural_buf);
                    let mut result = Self::read_keccak_merkle_layers_zero_copy(merkle);
                    result.leaf_keepalive = Some(dif_buf);
                    (out_values, result)
                }
                Some(false) => {
                ...
                None => {
                    Self::release_buf(&self.data_buf_cache, dif_buf);
                    Self::release_buf(&self.temp_buf_cache, natural_buf);
                    (out_values, Self::read_keccak_merkle_layers_zero_copy(merkle))
                }
            }
        })
    }
    ...
    fn encode_dif_stages_inplace(
        ...
        while dif_stage < log_n {
            ...
            if gap >= 5 {
                let num_units = height >> 5;
                if first_dispatch {
                    let max_tg = self.dif_r32_oop_ps.max_total_threads_per_threadgroup() as u32;
                    enc.set_compute_pipeline_state(&self.dif_r32_oop_ps);
                    ...
                    self.dispatch_dif_inplace(enc, data, twiddles, height, width, dif_stage, tg_w, &self.dif_r32_ps, 5);
                }
                dif_stage += 5;
            } else if gap >= 4 {
                ...
                if first_dispatch {
                    ...
                    self.dispatch_dif_inplace(enc, data, twiddles, height, width, dif_stage, tg_w, &self.dif_r16_ps, 4);
                }
                dif_stage += 4;
            } else if gap >= 3 {
        ...
        if inputs.len() == 1 {
            ...
            if total_bytes >= 128 * 1024 {
                ...
                let result = if is_contiguous {
                    ...
                    let rm = RowMajorMatrix::new(flat, width);
                    self.gpu.gpu_build_keccak_merkle_digests(&rm)
                };
        ...
        + Sync,
{
        ...
        EF: p3_field::ExtensionField<KoalaBear> + BasedVectorSpace<KoalaBear> + Clone + Send + Sync,
    {
        ...
        let (values, result) = self.gpu.gpu_transpose_dft_and_merkle_packed(
            packed_data.as_ptr().cast(),
            packed_data.len(),
            in_rows, in_cols, padded_height, d,
        )?;
        let ef_values = {
            let (len, cap) = (values.len(), values.capacity());
            let p = values.as_ptr() as *mut EF;
        ...
        + Sync,
{
    ...
    > {
        let height = mat.height();
        let width = mat.width();
        match self.gpu.gpu_dft_and_keccak_merkle(&mut mat.values, height, width) {
            Some(result) => Ok(result.into_tree(vec![mat])),
            None => Err(mat),
        }
    }
        ...
        EF: p3_field::ExtensionField<KoalaBear> + BasedVectorSpace<KoalaBear> + Clone + Send + Sync,
    {
        ...
        match self.gpu.gpu_dft_and_keccak_merkle(&mut base_values, height, base_width) {
            Some(result) => {
                let ef_mat = RowMajorMatrix::new(reconstitute_ef(base_values), init_width);
                let flat_view = p3_matrix::extension::FlatMatrixView::new(ef_mat);
                Ok(result.into_tree(vec![flat_view]))
            }
            None => Err(RowMajorMatrix::new(reconstitute_ef(base_values), init_width)),
        }
    }
        ...
        EF: p3_field::ExtensionField<KoalaBear> + BasedVectorSpace<KoalaBear> + Clone + Send + Sync,
    {
        ...
        let (values, result) = self.gpu.gpu_transpose_dft_and_keccak_merkle(
            base_data, in_rows, in_cols, padded_height, d,
        )?;
        let ef_values = {
            let (len, cap) = (values.len(), values.capacity());
            let p = values.as_ptr() as *mut EF;
        ...
        EF: p3_field::ExtensionField<KoalaBear> + BasedVectorSpace<KoalaBear> + Clone + Send + Sync,
    {
        let d = EF::DIMENSION;
        assert_eq!(packed_data.len(), num_ef * d);
        let (values, result) = self.gpu.gpu_transpose_dft_and_keccak_merkle_packed(
            packed_data.as_ptr().cast(),
            packed_data.len(),
            in_rows,
            in_cols,
