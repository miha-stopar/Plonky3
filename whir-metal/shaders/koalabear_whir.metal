// KoalaBear field + forward NTT + Poseidon2 Merkle hashing + PoW grinding.
...
// │ Kernel                      │ Line │ Purpose                                 │
// ├─────────────────────────────┼──────┼─────────────────────────────────────────┤
// │ kb_bandwidth_test           │    6 │ Read-write bandwidth microbenchmark     │
// │ kb_bitrev_gather            │   67 │ Bit-reversal permutation (gather)       │
// │ kb_ntt_bitrev               │   86 │ Bit-reversal + twiddle (legacy)         │
// │ kb_ntt_shared_mem           │  121 │ Shared-memory radix-2 NTT block         │
// │ kb_ntt_shared_mem_gs        │  204 │ Shared-memory Gentleman-Sande NTT       │
// │ kb_ntt_butterfly            │  303 │ Global radix-2 butterfly                │
// │ kb_ntt_butterfly_r4         │  345 │ Global radix-4 butterfly                │
// │ kb_ntt_butterfly_r8         │  418 │ Global radix-8 butterfly                │
// │ kb_ntt_stockham             │  509 │ Stockham auto-sort NTT block            │
// │ kb_stockham_global_r2       │  656 │ Global Stockham radix-2 pass            │
// ├─────────────────────────────┼──────┼─────────────────────────────────────────┤
// │ kb_dif_r8                   │  707 │ DIF radix-8 in-place stage              │
// │ kb_dif_r4                   │  806 │ DIF radix-4 in-place stage              │
// │ kb_dif_r2                   │  869 │ DIF radix-2 in-place stage              │
// │ kb_dif_r16                  │  904 │ DIF radix-16 in-place stage             │
// │ kb_dif_r16_oop              │  992 │ DIF radix-16 out-of-place (first pass)  │
// │ kb_dif_r32                  │ 1077 │ DIF radix-32 in-place stage             │
// │ kb_dif_r32_bitrev           │ 1180 │ DIF radix-32 + bit-reversal gather      │
// │ kb_dif_r16_bitrev           │ 1270 │ DIF radix-16 + bit-reversal gather      │
// │ kb_dif_r8_bitrev            │ 1368 │ DIF radix-8  + bit-reversal gather      │
// │ kb_dif_r4_bitrev            │ 1471 │ DIF radix-4  + bit-reversal gather      │
// │ kb_dif_r2_bitrev            │ 1536 │ DIF radix-2  + bit-reversal gather      │
// │ kb_dif_shared_bitrev        │ 1652 │ Shared-memory DIF + bitrev (small n)    │
// ├─────────────────────────────┼──────┼─────────────────────────────────────────┤
// │ kb_transpose                │ 1578 │ Matrix transpose                        │
// │ kb_ntt_twiddle_transpose    │ 1602 │ Twiddle multiply + transpose            │
// ├─────────────────────────────┼──────┼─────────────────────────────────────────┤
...
constant uint KB_P = 0x7f000001u;

// P^{-1} mod 2^32 — the Montgomery constant for KoalaBear.
constant uint KB_MONTY_MU = 0x81000001u;
...
Kb kb_add(Kb a, Kb b) {
    uint sum = a.v + b.v;
    return Kb{sum >= KB_P ? sum - KB_P : sum};
}

Kb kb_sub(Kb a, Kb b) {
    if (a.v >= b.v) return Kb{a.v - b.v};
    return Kb{a.v + KB_P - b.v};
}
...
Kb kb_mul(Kb a, Kb b) {
    ...
    uint t = x_lo * KB_MONTY_MU;
    uint u_hi = mulhi(t, KB_P);
    uint hi = x_hi - u_hi;
    return Kb{u_hi > x_hi ? hi + KB_P : hi};
}

Kb kb_neg(Kb a) { return Kb{a.v == 0 ? 0u : KB_P - a.v}; }
...
kernel void kb_bitrev_gather(
    device const Kb* src           [[buffer(0)]],
    device Kb* dst                 [[buffer(1)]],
...
kernel void kb_ntt_bitrev(
    device Kb* data                [[buffer(0)]],
...
kernel void kb_ntt_shared_mem(
    device Kb* data                [[buffer(0)]],
    device const Kb* twiddles      [[buffer(1)]],
    constant uint& height          [[buffer(2)]],
    constant uint& width           [[buffer(3)]],
            ...
            if ((tid & stride) == 0) {
                ...
                if (tw_idx == 0) {
                    for (uint c = 0; c < cw; c++) {
                        uint off = c * tg_size;
                        Kb a = sdata[off + tid];
                        Kb b = sdata[off + partner];
                        sdata[off + tid] = kb_add(a, b);
                        sdata[off + partner] = kb_sub(a, b);
                    }
                } else {
                    Kb w = twiddles[tw_idx];
                    for (uint c = 0; c < cw; c++) {
                        uint off = c * tg_size;
                        Kb a = sdata[off + tid];
                        Kb b = sdata[off + partner];
                        Kb wb = kb_mul(w, b);
                        sdata[off + tid] = kb_add(a, wb);
                        sdata[off + partner] = kb_sub(a, wb);
                    }
                }
            }
...
// Like kb_ntt_shared_mem but reads from `input` and writes to `output`
...
kernel void kb_ntt_shared_mem_gs(
    device const Kb* input         [[buffer(0)]],
    device Kb* output              [[buffer(1)]],
    device const Kb* twiddles      [[buffer(2)]],
    ...
    if (apply_twiddle) {
        ...
        if (tw_raw != 0) {
            ...
            if (tw_raw >= half_height) {
                tw_factor = kb_neg(twiddles[tw_raw - half_height]);
            } else {
    ...
    for (uint col_base = 0; col_base < width; col_base += tile_w) {
        ...
        for (uint c = 0; c < cw; c++) {
            Kb val = input[gm_load + col_base + c];
            if (has_twiddle) val = kb_mul(tw_factor, val);
...
kernel void kb_ntt_butterfly(
    ...
    Kb b = data[idx_b];
    if (twiddle_idx == 0) {
        data[idx_a] = kb_add(a, b);
        data[idx_b] = kb_sub(a, b);
    } else {
        Kb w = twiddles[twiddle_idx];
        Kb wb = kb_mul(w, b);
        data[idx_a] = kb_add(a, wb);
        data[idx_b] = kb_sub(a, wb);
    }
}
...
kernel void kb_ntt_butterfly_r4(
    device Kb* data                [[buffer(0)]],
    device const Kb* twiddles      [[buffer(1)]],
    constant uint& height          [[buffer(2)]],
    constant uint& width           [[buffer(3)]],
    ...
    if (tw2_idx == 0) {
        ...
        wC = kb_mul(twiddles[tw2_idx], C);
    }
    wD = kb_mul(twiddles[tw3_idx], D);

    data[idx0] = kb_add(A, wC);
    data[idx1] = kb_add(B, wD);
    data[idx2] = kb_sub(A, wC);
    data[idx3] = kb_sub(B, wD);
}
...
kernel void kb_ntt_butterfly_r8(
    device Kb* data                [[buffer(0)]],
    device const Kb* twiddles      [[buffer(1)]],
    constant uint& height          [[buffer(2)]],
    constant uint& width           [[buffer(3)]],
    ...
    Kb e0 = data[(base              ) * width + col];
    Kb e1 = data[(base +     half_s ) * width + col];
    Kb e2 = data[(base + 2 * half_s ) * width + col];
    Kb e3 = data[(base + 3 * half_s ) * width + col];
    Kb e4 = data[(base + 4 * half_s ) * width + col];
    Kb e5 = data[(base + 5 * half_s ) * width + col];
    Kb e6 = data[(base + 6 * half_s ) * width + col];
    Kb e7 = data[(base + 7 * half_s ) * width + col];

    // ── Stage s: 4 butterflies, stride = half_s ──
    uint tw_s = j * (height >> (stage + 1));
    if (tw_s != 0) {
        Kb w = twiddles[tw_s];
        e1 = kb_mul(w, e1); e3 = kb_mul(w, e3);
        e5 = kb_mul(w, e5); e7 = kb_mul(w, e7);
    }
    Kb a0 = kb_add(e0, e1); Kb a1 = kb_sub(e0, e1);
    Kb a2 = kb_add(e2, e3); Kb a3 = kb_sub(e2, e3);
    Kb a4 = kb_add(e4, e5); Kb a5 = kb_sub(e4, e5);
    Kb a6 = kb_add(e6, e7); Kb a7 = kb_sub(e6, e7);
    ...
    if (tw1a != 0) {
        Kb wa = twiddles[tw1a];
        a2 = kb_mul(wa, a2); a6 = kb_mul(wa, a6);
    }
    { Kb wb = twiddles[tw1b];
      a3 = kb_mul(wb, a3); a7 = kb_mul(wb, a7); }
    Kb b0 = kb_add(a0, a2); Kb b2 = kb_sub(a0, a2);
    Kb b1 = kb_add(a1, a3); Kb b3 = kb_sub(a1, a3);
    Kb b4 = kb_add(a4, a6); Kb b6 = kb_sub(a4, a6);
    Kb b5 = kb_add(a5, a7); Kb b7 = kb_sub(a5, a7);

    // ── Stage s+2: 4 butterflies, stride = 4·half_s ──
    // Pairs: (b0,b4) (b1,b5) (b2,b6) (b3,b7)
    uint step2 = height >> (stage + 3);
    uint t0 = j * step2;
    if (t0 != 0) b4 = kb_mul(twiddles[t0], b4);
    b5 = kb_mul(twiddles[(j +     half_s) * step2], b5);
    b6 = kb_mul(twiddles[(j + 2 * half_s) * step2], b6);
    b7 = kb_mul(twiddles[(j + 3 * half_s) * step2], b7);

    data[(base              ) * width + col] = kb_add(b0, b4);
    data[(base +     half_s ) * width + col] = kb_add(b1, b5);
    data[(base + 2 * half_s ) * width + col] = kb_add(b2, b6);
    data[(base + 3 * half_s ) * width + col] = kb_add(b3, b7);
    data[(base + 4 * half_s ) * width + col] = kb_sub(b0, b4);
    data[(base + 5 * half_s ) * width + col] = kb_sub(b1, b5);
    data[(base + 6 * half_s ) * width + col] = kb_sub(b2, b6);
    data[(base + 7 * half_s ) * width + col] = kb_sub(b3, b7);
}
...
kernel void kb_ntt_stockham(
    ...
    for (uint col_base = 0; col_base < width; col_base += tile_w) {
        ...
        for (uint c = 0; c < cw; c++) {
            ...
            Kb r0 = data[(base_row + tid              ) * width + gm];
            Kb r1 = data[(base_row + tid +     quarter) * width + gm];
            Kb r2 = data[(base_row + tid + 2 * quarter) * width + gm];
            Kb r3 = data[(base_row + tid + 3 * quarter) * width + gm];

            // Radix-4 DIT butterfly (no twiddle for pass 0)
            Kb t0 = kb_add(r0, r2);
            Kb t1 = kb_sub(r0, r2);
            Kb t2 = kb_add(r1, r3);
            Kb t3 = kb_mul(omega_4, kb_sub(r1, r3));

            // Stockham write (stride=1): wr = tid * 4 + j
            uint wr = tid << 2;
            sdata[sm + wr    ] = kb_add(t0, t2);
            sdata[sm + wr + 1] = kb_add(t1, t3);
            sdata[sm + wr + 2] = kb_sub(t0, t2);
            sdata[sm + wr + 3] = kb_sub(t1, t3);
        }
        threadgroup_barrier(mem_flags::mem_threadgroup);
        ...
        for (uint pass = 1; pass < n_passes; pass++) {
            ...
            for (uint c = 0; c < cw; c++) {
                uint sm = c * block_size;
                Kb r0 = sdata[sm + tid              ];
                Kb r1 = sdata[sm + tid +     quarter];
                Kb r2 = sdata[sm + tid + 2 * quarter];
                Kb r3 = sdata[sm + tid + 3 * quarter];
                ...
                sdata[sm + wr              ] = kb_add(t0, t2);
                sdata[sm + wr +     stride ] = kb_add(t1, t3);
                sdata[sm + wr + 2 * stride ] = kb_sub(t0, t2);
                sdata[sm + wr + 3 * stride ] = kb_sub(t1, t3);
        ...
        if (log_block & 1u) {
            ...
            for (uint c = 0; c < cw; c++) {
                ...
                for (uint pair = 0; pair < 2; pair++) {
                    ...
                    Kb r0 = sdata[sm + k             ];
                    Kb r1 = sdata[sm + k + half_block];

                    uint tw_idx = k * tw_step;
                    if (tw_idx != 0) {
                        r1 = kb_mul(twiddles[tw_idx], r1);
                    }
                    sdata[sm + k             ] = kb_add(r0, r1);
                    sdata[sm + k + half_block] = kb_sub(r0, r1);
                }
            }
            threadgroup_barrier(mem_flags::mem_threadgroup);
        }
...
// Like kb_ntt_stockham but reads from `input` and writes to `output`
...
kernel void kb_ntt_stockham_gs(
    device const Kb* input          [[buffer(0)]],
    device Kb* output               [[buffer(1)]],
    device const Kb* twiddles       [[buffer(2)]],
    constant uint& height           [[buffer(3)]],
    ...
    if (apply_twiddle) {
        for (uint j = 0; j < 4; j++) {
            ...
            if (tw_raw != 0) {
                has_twiddle[j] = true;
                if (tw_raw >= half_height) {
                    tw_f[j] = kb_neg(twiddles[tw_raw - half_height]);
                } else {
    ...
    for (uint col_base = 0; col_base < width; col_base += tile_w) {
        ...
        for (uint c = 0; c < cw; c++) {
            ...
            if (apply_twiddle) {
                if (has_twiddle[0]) r0 = kb_mul(tw_f[0], r0);
                if (has_twiddle[1]) r1 = kb_mul(tw_f[1], r1);
                if (has_twiddle[2]) r2 = kb_mul(tw_f[2], r2);
                if (has_twiddle[3]) r3 = kb_mul(tw_f[3], r3);
            }
        ...
        for (uint pass = 1; pass < n_passes; pass++) {
            ...
            for (uint c = 0; c < cw; c++) {
                uint sm = c * block_size;
                Kb r0_v = sdata[sm + tid              ];
                Kb r1_v = sdata[sm + tid +     quarter];
                Kb r2_v = sdata[sm + tid + 2 * quarter];
                Kb r3_v = sdata[sm + tid + 3 * quarter];

                r1_v = kb_mul(tw1, r1_v);
                r2_v = kb_mul(tw2, r2_v);
                r3_v = kb_mul(tw3, r3_v);

                Kb t0_v = kb_add(r0_v, r2_v);
                Kb t1_v = kb_sub(r0_v, r2_v);
                Kb t2_v = kb_add(r1_v, r3_v);
                Kb t3_v = kb_mul(omega_4, kb_sub(r1_v, r3_v));

                threadgroup_barrier(mem_flags::mem_threadgroup);

                sdata[sm + wr              ] = kb_add(t0_v, t2_v);
                sdata[sm + wr +     stride ] = kb_add(t1_v, t3_v);
                sdata[sm + wr + 2 * stride ] = kb_sub(t0_v, t2_v);
                sdata[sm + wr + 3 * stride ] = kb_sub(t1_v, t3_v);
        ...
        if (log_block & 1u) {
            ...
            for (uint c = 0; c < cw; c++) {
                ...
                for (uint pair = 0; pair < 2; pair++) {
                    uint k = tid + pair * quarter;
                    Kb r0_v = sdata[sm + k             ];
                    Kb r1_v = sdata[sm + k + half_block];

                    uint tw_idx = k * tw_step_f;
                    if (tw_idx != 0) {
                        r1_v = kb_mul(twiddles[tw_idx], r1_v);
                    }
                    sdata[sm + k             ] = kb_add(r0_v, r1_v);
                    sdata[sm + k + half_block] = kb_sub(r0_v, r1_v);
                }
            }
            threadgroup_barrier(mem_flags::mem_threadgroup);
        }
...
kernel void kb_stockham_global_r2(
    device const Kb* src       [[buffer(0)]],
    device Kb* dst             [[buffer(1)]],
    device const Kb* twiddles  [[buffer(2)]],
    constant uint& height      [[buffer(3)]],
    ...
    Kb a = src[tid * width + col];
    Kb b = src[(tid + half_n) * width + col];

    uint tw_idx = pos * (half_n / p);
    if (tw_idx != 0) {
        b = kb_mul(twiddles[tw_idx], b);
    }

    uint dst_a = group * 2 * p + pos;
    uint dst_b = dst_a + p;
    dst[dst_a * width + col] = kb_add(a, b);
    dst[dst_b * width + col] = kb_sub(a, b);
}
...
kernel void kb_dif_r8(
    ...
    Kb e0 = data[(base          ) * width + col];
    Kb e1 = data[(base +     h  ) * width + col];
    Kb e2 = data[(base + 2 * h  ) * width + col];
    Kb e3 = data[(base + 3 * h  ) * width + col];
    Kb e4 = data[(base + 4 * h  ) * width + col];
    Kb e5 = data[(base + 5 * h  ) * width + col];
    Kb e6 = data[(base + 6 * h  ) * width + col];
    Kb e7 = data[(base + 7 * h  ) * width + col];
    ...
    // ── DIF stage dif_stage: stride = 4h, pairs (0,4)(1,5)(2,6)(3,7) ──
    {
        ...
        Kb s0 = kb_add(e0, e4); Kb d0 = kb_sub(e0, e4);
        Kb s1 = kb_add(e1, e5); Kb d1 = kb_sub(e1, e5);
        Kb s2 = kb_add(e2, e6); Kb d2 = kb_sub(e2, e6);
        Kb s3 = kb_add(e3, e7); Kb d3 = kb_sub(e3, e7);

        e0 = s0; e1 = s1; e2 = s2; e3 = s3;
        e4 = (tw0_idx == 0) ? d0 : kb_mul(twiddles[tw0_idx], d0);
        e5 = kb_mul(twiddles[tw1_idx], d1);
        e6 = kb_mul(twiddles[tw2_idx], d2);
        e7 = kb_mul(twiddles[tw3_idx], d3);
    }
    ...
    // ── DIF stage dif_stage+1: stride = 2h, pairs (0,2)(1,3)(4,6)(5,7) ──
    {
        ...
        Kb s0 = kb_add(e0, e2); Kb d0 = kb_sub(e0, e2);
        Kb s1 = kb_add(e1, e3); Kb d1 = kb_sub(e1, e3);
        Kb s4 = kb_add(e4, e6); Kb d4 = kb_sub(e4, e6);
        Kb s5 = kb_add(e5, e7); Kb d5 = kb_sub(e5, e7);

        e0 = s0; e1 = s1; e4 = s4; e5 = s5;
        e2 = (tw4_idx == 0) ? d0 : kb_mul(twiddles[tw4_idx], d0);
        e3 = kb_mul(twiddles[tw5_idx], d1);
        e6 = (tw4_idx == 0) ? d4 : kb_mul(twiddles[tw4_idx], d4);
        e7 = kb_mul(twiddles[tw5_idx], d5);
    }
    ...
    // ── DIF stage dif_stage+2: stride = h, pairs (0,1)(2,3)(4,5)(6,7) ──
    {
        ...
        Kb s0 = kb_add(e0, e1); Kb d0 = kb_sub(e0, e1);
        Kb s2 = kb_add(e2, e3); Kb d2 = kb_sub(e2, e3);
        Kb s4 = kb_add(e4, e5); Kb d4 = kb_sub(e4, e5);
        Kb s6 = kb_add(e6, e7); Kb d6 = kb_sub(e6, e7);

        e0 = s0; e2 = s2; e4 = s4; e6 = s6;
        if (tw6_idx == 0) {
            e1 = d0; e3 = d2; e5 = d4; e7 = d6;
        } else {
            Kb w = twiddles[tw6_idx];
            e1 = kb_mul(w, d0); e3 = kb_mul(w, d2);
            e5 = kb_mul(w, d4); e7 = kb_mul(w, d6);
        }
    }
...
kernel void kb_dif_r4(
    ...
    // DIF stage dif_stage: stride = 2h, pairs (0,2)(1,3)
    {
        ...
        e0 = s0; e1 = s1;
        e2 = (tw0_idx == 0) ? d0 : kb_mul(twiddles[tw0_idx], d0);
        e3 = kb_mul(twiddles[tw1_idx], d1);
    }
...
kernel void kb_dif_r16(
    ...
    // ── DIF stage dif_stage: stride = 8h, pairs (k, k+8) for k=0..7 ──
    {
        for (uint k = 0; k < 8; k++) {
            ...
            Kb sum = kb_add(e[k], e[k + 8]);
            Kb diff = kb_sub(e[k], e[k + 8]);
            e[k] = sum;
            e[k + 8] = (tw_idx == 0) ? diff : kb_mul(twiddles[tw_idx], diff);
        }
    }
    ...
    // ── DIF stage dif_stage+1: stride = 4h ──
    {
        ...
        for (uint hh = 0; hh < 2; hh++) {
            ...
            for (uint k = 0; k < 4; k++) {
                ...
                Kb sum = kb_add(e[off + k], e[off + k + 4]);
                Kb diff = kb_sub(e[off + k], e[off + k + 4]);
                e[off + k] = sum;
                e[off + k + 4] = (tw_idx == 0) ? diff : kb_mul(twiddles[tw_idx], diff);
            }
        }
    }
    ...
    // ── DIF stage dif_stage+2: stride = 2h ──
    {
        ...
        for (uint q = 0; q < 4; q++) {
            ...
            for (uint k = 0; k < 2; k++) {
                ...
                Kb sum = kb_add(e[off + k], e[off + k + 2]);
                Kb diff = kb_sub(e[off + k], e[off + k + 2]);
                e[off + k] = sum;
                e[off + k + 2] = (tw_idx == 0) ? diff : kb_mul(twiddles[tw_idx], diff);
            }
        }
    }
    ...
    // ── DIF stage dif_stage+3: stride = h ──
    {
        ...
        for (uint p = 0; p < 8; p++) {
            ...
            Kb sum = kb_add(e[off], e[off + 1]);
            Kb diff = kb_sub(e[off], e[off + 1]);
            e[off] = sum;
            e[off + 1] = (tw_idx == 0) ? diff : kb_mul(twiddles[tw_idx], diff);
        }
    }
...
kernel void kb_dif_r32(
    ...
    // Stage 0: stride = 16h, pairs (k, k+16)
    {
        ...
        for (uint k = 0; k < 16; k++) {
            ...
            Kb sum = kb_add(e[k], e[k + 16]);
            Kb diff = kb_sub(e[k], e[k + 16]);
            e[k] = sum;
            e[k + 16] = (tw_idx == 0) ? diff : kb_mul(twiddles[tw_idx], diff);
        }
    }
    ...
    // Stage 1: stride = 8h, pairs within 2 halves of 16
    {
        ...
        for (uint hh = 0; hh < 2; hh++) {
            ...
            for (uint k = 0; k < 8; k++) {
                ...
                Kb sum = kb_add(e[off + k], e[off + k + 8]);
                Kb diff = kb_sub(e[off + k], e[off + k + 8]);
                e[off + k] = sum;
                e[off + k + 8] = (tw_idx == 0) ? diff : kb_mul(twiddles[tw_idx], diff);
            }
        }
    }
...
// Same as kb_dif_r8 but reads from `src` and writes to `dst` at
...
kernel void kb_dif_r8_bitrev(
    ...
    Kb e0 = src[(base          ) * width + col];
    Kb e1 = src[(base +     h  ) * width + col];
    Kb e2 = src[(base + 2 * h  ) * width + col];
    Kb e3 = src[(base + 3 * h  ) * width + col];
    Kb e4 = src[(base + 4 * h  ) * width + col];
    Kb e5 = src[(base + 5 * h  ) * width + col];
    Kb e6 = src[(base + 6 * h  ) * width + col];
    Kb e7 = src[(base + 7 * h  ) * width + col];
    ...
    Kb vals[8] = {e0, e1, e2, e3, e4, e5, e6, e7};
    for (uint i = 0; i < 8; i++) {
        uint rev_row = bit_reverse_n(rows[i], log_n);
...
kernel void kb_dif_r2_bitrev(
    ...
    Kb a = src[i * width + col];
    Kb b = src[j_pos * width + col];
    ...
    uint tw_idx = k * (1u << dif_stage);
    Kb result_a = sum;
    Kb result_b = (tw_idx == 0) ? diff : kb_mul(twiddles[tw_idx], diff);
...
// 2D grid: gid.x = column, gid.y = row (0..N-1).
kernel void kb_ntt_twiddle_transpose(
    device const Kb* src           [[buffer(0)]],
    device Kb* dst                 [[buffer(1)]],
    device const Kb* twiddles      [[buffer(2)]],
    ...
    Kb val = src[row * width + col];

    uint tw_raw = blk * pos;
    if (tw_raw != 0) {
        bool neg = tw_raw >= half_n;
        Kb tw = twiddles[neg ? tw_raw - half_n : tw_raw];
        if (neg) tw = kb_neg(tw);
        val = kb_mul(val, tw);
    }
...
kernel void kb_dif_shared_bitrev(
    device const Kb* src       [[buffer(0)]],
    device Kb* dst             [[buffer(1)]],
    device const Kb* twiddles  [[buffer(2)]],
    constant uint& height      [[buffer(3)]],
    ...
    for (uint ls = 0; ls < log_block; ls++) {
        ...
        uint tw_idx = k << s;
        sdata[idx_a] = sum;
        sdata[idx_b] = (tw_idx == 0) ? diff : kb_mul(twiddles[tw_idx], diff);
...
// Same interface as kb_dif_shared_bitrev but 4× larger blocks.
kernel void kb_dif_shared_bitrev_lg(
    device const Kb* src       [[buffer(0)]],
    device Kb* dst             [[buffer(1)]],
    device const Kb* twiddles  [[buffer(2)]],
    constant uint& height      [[buffer(3)]],
...
kernel void kb_dif_shared_bitrev_xl(
    device const Kb* src       [[buffer(0)]],
    device Kb* dst             [[buffer(1)]],
    device const Kb* twiddles  [[buffer(2)]],
    constant uint& height      [[buffer(3)]],
...
// ═══════════════════════════════════════════════════════════════════════
// Poseidon2 KoalaBear width-16 permutation + Merkle hashing kernels
// ═══════════════════════════════════════════════════════════════════════
//
...
Kb kb_double(Kb a) { return kb_add(a, a); }

Kb kb_halve(Kb a) {
    uint v = a.v;
    return Kb{(v & 1u) ? ((v >> 1) + ((KB_P >> 1) + 1)) : (v >> 1)};
}
...
// `elem_size` is the number of Kb words per logical element (1 for base
// field, 4 for quartic extension field).
// Each thread handles one logical element of the output.
kernel void kb_transpose_pad(
    device const Kb* src       [[buffer(0)]],
    device Kb* dst             [[buffer(1)]],
...
// Variant of kb_transpose_pad that reads from NEON packed (SoA) layout.
// In packed form, 4 EF values share a block of elem_size*4 Kb values:
//   block[comp * 4 + lane] = EF[lane].component[comp]
// The output is identical to kb_transpose_pad (AoS layout, transposed).
kernel void kb_transpose_pad_packed(
    device const Kb* src       [[buffer(0)]],
    device Kb* dst             [[buffer(1)]],
    constant uint& in_rows     [[buffer(2)]],
    constant uint& in_cols     [[buffer(3)]],
...
Kb kb_sbox(Kb x) {
    ...
    Kb x3 = kb_mul(x2, x);
    Kb x4 = kb_mul(x2, x2);
    return kb_mul(x4, x3);     // x^7
}

// 4×4 MDS matrix: [[2,3,1,1],[1,2,3,1],[1,1,2,3],[3,1,1,2]]
void apply_mat4(thread Kb* x) {
    Kb t01   = kb_add(x[0], x[1]);
    Kb t23   = kb_add(x[2], x[3]);
    Kb t0123 = kb_add(t01, t23);
    Kb t01123 = kb_add(t0123, x[1]);
    Kb t01233 = kb_add(t0123, x[3]);
    Kb new3 = kb_add(t01233, kb_double(x[0]));
    Kb new1 = kb_add(t01123, kb_double(x[2]));
    x[0] = kb_add(t01123, t01);
    x[2] = kb_add(t01233, t23);
    x[1] = new1;
    x[3] = new3;
}
...
void mds_light_16(thread Kb* s) {
    ...
    Kb sums[4];
    for (uint k = 0; k < 4; k++) {
        sums[k] = kb_add(kb_add(s[k], s[k + 4]), kb_add(s[k + 8], s[k + 12]));
    }
    for (uint i = 0; i < 16; i++) {
        s[i] = kb_add(s[i], sums[i & 3]);
    }
}

Kb kb_div_2exp(Kb a, uint k) {
    for (uint i = 0; i < k; i++) a = kb_halve(a);
    return a;
}

// Internal linear layer for KoalaBear width-16.
void internal_layer_16(thread Kb* s, Kb full_sum, Kb inv256, Kb inv2_27) {
    (void)inv256;
    (void)inv2_27;
    s[1]  = kb_add(s[1], full_sum);
    s[2]  = kb_add(kb_double(s[2]), full_sum);
    s[3]  = kb_add(kb_halve(s[3]), full_sum);
    s[4]  = kb_add(full_sum, kb_add(kb_double(s[4]), s[4]));
    s[5]  = kb_add(full_sum, kb_double(kb_double(s[5])));
    s[6]  = kb_sub(full_sum, kb_halve(s[6]));
    s[7]  = kb_sub(full_sum, kb_add(kb_double(s[7]), s[7]));
    s[8]  = kb_sub(full_sum, kb_double(kb_double(s[8])));
    s[9]  = kb_add(kb_div_2exp(s[9], 8), full_sum);
    s[10] = kb_add(kb_div_2exp(s[10], 3), full_sum);
    s[11] = kb_add(kb_div_2exp(s[11], 24), full_sum);
    s[12] = kb_sub(full_sum, kb_div_2exp(s[12], 8));
    s[13] = kb_sub(full_sum, kb_div_2exp(s[13], 3));
    s[14] = kb_sub(full_sum, kb_div_2exp(s[14], 4));
    s[15] = kb_sub(full_sum, kb_div_2exp(s[15], 24));
}

// Full Poseidon2 permutation: width=16, R_F=8 (4+4), R_P=13, S-box=x^7.
void poseidon2_permute_16(thread Kb* state, constant Kb* rc) {
    Kb inv256  = rc[141];
    Kb inv2_27 = rc[142];
    ...
    constant Kb* ext_init = rc;
    for (uint r = 0; r < 4; r++) {
        for (uint i = 0; i < 16; i++) {
            state[i] = kb_sbox(kb_add(state[i], ext_init[r * 16 + i]));
        }
        mds_light_16(state);
    }

    // 13 partial (internal) rounds
    constant Kb* int_rc = rc + 64;
    for (uint r = 0; r < 13; r++) {
        state[0] = kb_sbox(kb_add(state[0], int_rc[r]));

        Kb part_sum = state[1];
        for (uint i = 2; i < 16; i++) part_sum = kb_add(part_sum, state[i]);
        Kb full_sum = kb_add(part_sum, state[0]);
        state[0] = kb_sub(part_sum, state[0]);

        internal_layer_16(state, full_sum, inv256, inv2_27);
    }

    // 4 terminal full rounds
    constant Kb* ext_term = rc + 77;
    for (uint r = 0; r < 4; r++) {
        for (uint i = 0; i < 16; i++) {
            state[i] = kb_sbox(kb_add(state[i], ext_term[r * 16 + i]));
        }
        mds_light_16(state);
    }
}
...
kernel void poseidon2_hash_leaves(
    device const Kb* data        [[buffer(0)]],
    device Kb* digests           [[buffer(1)]],
    constant Kb* rc              [[buffer(2)]],
    ...
    for (uint absorbed = 0; absorbed < leaf_width; absorbed += 8) {
        ...
        if (remaining < 8) {
            for (uint i = remaining; i < 8; i++) state[i] = Kb{0};
        }
...
kernel void poseidon2_merkle_compress(
    device const Kb* children    [[buffer(0)]],
    device Kb* parents           [[buffer(1)]],
    constant Kb* rc              [[buffer(2)]],
...
kernel void poseidon2_hash_and_compress(
    device const Kb* data        [[buffer(0)]],
    device Kb* leaf_digests      [[buffer(1)]],
    device Kb* parents           [[buffer(2)]],
    constant Kb* rc              [[buffer(3)]],
    ...
    for (uint absorbed = 0; absorbed < leaf_width; absorbed += 8) {
        ...
        if (remaining < 8) {
            for (uint i = remaining; i < 8; i++) left[i] = Kb{0};
        }
    ...
    for (uint absorbed = 0; absorbed < leaf_width; absorbed += 8) {
        ...
        if (remaining < 8) {
            for (uint i = remaining; i < 8; i++) right[i] = Kb{0};
        }
...
kernel void poseidon2_hash4_compress3(
    ...
    device Kb* leaf_digests      [[buffer(1)]],
    device Kb* parents           [[buffer(2)]],
    device Kb* grandparents      [[buffer(3)]],
    constant Kb* rc              [[buffer(4)]],
    ...
    // Hash leaf 4*gid+0
    {
        ...
        for (uint absorbed = 0; absorbed < leaf_width; absorbed += 8) {
            ...
            if (rem < 8) for (uint i = rem; i < 8; i++) st[i] = Kb{0};
...
kernel void poseidon2_merkle_simd(
    ...
    for (uint level = 1; level <= 5; level++) {
        ...
        for (uint i = 0; i < 8; i++)
            st[8 + i] = Kb{simd_shuffle_down(st[i].v, delta)};
...
// ── Keccak (u64-lane) Merkle hashing kernels ──────────────────────────
...
inline void keccakf_1600(thread ulong a[25]) {
    for (uint round = 0; round < 24; round++) {
        ...
        for (uint t = 0; t < 24; t++) {
            ...
            uint nx = y;
            uint ny = (2 * x + 3 * y) % 5;
            b[nx + 5 * ny] = rol64(a[idx], KECCAK_ROT[idx]);
            x = nx; y = ny;
        }

        for (uint yy = 0; yy < 5; yy++) {
            uint o = 5 * yy;
            ulong b0 = b[o + 0], b1 = b[o + 1], b2 = b[o + 2], b3 = b[o + 3], b4 = b[o + 4];
            a[o + 0] = b0 ^ ((~b1) & b2);
            a[o + 1] = b1 ^ ((~b2) & b3);
            a[o + 2] = b2 ^ ((~b3) & b4);
            a[o + 3] = b3 ^ ((~b4) & b0);
...
// Matches Plonky3 `SerializingHasher` + `PaddingFreeSponge<KeccakF,25,17,4>` on KoalaBear rows.
inline void keccak_hash_leaf_digest(
    device const uint* data,
    uint base,
    uint leaf_width,
    thread ulong digest[4]
) {
    ...
    uint i32 = 0;
    uint rate_idx = 0;
    while (i32 < leaf_width) {
        ulong lo = (ulong)data[base + i32];
        i32++;
        ulong hi = 0ul;
        if (i32 < leaf_width) {
            hi = (ulong)data[base + i32];
            i32++;
        }
        state[rate_idx] = lo | (hi << 32);
        rate_idx++;
        if (rate_idx == 17) {
            keccakf_1600(state);
            rate_idx = 0;
...
inline void keccak_compress_two_digests(
    ...
    thread ulong state[25];
    for (uint i = 0; i < 25; i++) state[i] = 0ul;
    for (uint i = 0; i < 4; i++) state[i] = a[i];
    for (uint i = 0; i < 4; i++) state[4 + i] = b[i];
    keccakf_1600(state);
    out[0] = state[0];
    out[1] = state[1];
    out[2] = state[2];
...
kernel void keccak_hash_leaves(
    device const uint* data          [[buffer(0)]],
    device ulong*      out_digests   [[buffer(1)]],
    ...
    uint base = gid * leaf_width;
    thread ulong dig[4];
    keccak_hash_leaf_digest(data, base, leaf_width, dig);
    uint out_base = gid * 4;
    for (uint i = 0; i < 4; i++) out_digests[out_base + i] = dig[i];
}

// Fused: hash two adjacent leaves + first Merkle compress (same layout as Poseidon path).
kernel void keccak_hash_and_compress(
    device const uint* data        [[buffer(0)]],
    device ulong*      leaf_out    [[buffer(1)]],
    device ulong*      parent_out  [[buffer(2)]],
    ...
    ulong d0[4], d1[4];
    keccak_hash_leaf_digest(data, (2 * gid) * leaf_width, leaf_width, d0);
    keccak_hash_leaf_digest(data, (2 * gid + 1) * leaf_width, leaf_width, d1);
    ...
    ulong p[4];
    keccak_compress_two_digests(d0, d1, p);
    uint po = gid * 4;
    for (uint i = 0; i < 4; i++) parent_out[po + i] = p[i];
}

// Fused 4-leaf hash + two compress rounds (parents + grandparents), mirroring poseidon2_hash4_compress3.
kernel void keccak_hash4_compress3(
    device const uint* data           [[buffer(0)]],
    device ulong*      leaf_digests   [[buffer(1)]],
    device ulong*      parents        [[buffer(2)]],
    device ulong*      grandparents   [[buffer(3)]],
    constant uint&     num_quads      [[buffer(4)]],
    constant uint&     leaf_width     [[buffer(5)]],
    uint gid [[thread_position_in_grid]]
) {
    if (gid >= num_quads) return;

    ulong d0[4], d1[4], d2[4], d3[4];
    keccak_hash_leaf_digest(data, (4 * gid) * leaf_width, leaf_width, d0);
    keccak_hash_leaf_digest(data, (4 * gid + 1) * leaf_width, leaf_width, d1);
    keccak_hash_leaf_digest(data, (4 * gid + 2) * leaf_width, leaf_width, d2);
    keccak_hash_leaf_digest(data, (4 * gid + 3) * leaf_width, leaf_width, d3);
    device ulong4* ld = (device ulong4*)&leaf_digests[(4 * gid) * 4];
    ld[0] = ulong4(d0[0], d0[1], d0[2], d0[3]);
    ld[1] = ulong4(d1[0], d1[1], d1[2], d1[3]);
    ld[2] = ulong4(d2[0], d2[1], d2[2], d2[3]);
    ld[3] = ulong4(d3[0], d3[1], d3[2], d3[3]);

    ulong p01[4];
    keccak_compress_two_digests(d0, d1, p01);
    uint out01 = (2 * gid) * 4;
    *((device ulong4*)&parents[out01]) = ulong4(p01[0], p01[1], p01[2], p01[3]);

    ulong p23[4];
    keccak_compress_two_digests(d2, d3, p23);
    uint out23 = (2 * gid + 1) * 4;
    *((device ulong4*)&parents[out23]) = ulong4(p23[0], p23[1], p23[2], p23[3]);

    ulong gp[4];
    keccak_compress_two_digests(p01, p23, gp);
    uint og = gid * 4;
    *((device ulong4*)&grandparents[og]) = ulong4(gp[0], gp[1], gp[2], gp[3]);
}

kernel void keccak_merkle_compress(
    device const ulong* in_digests [[buffer(0)]],
    device ulong*       out_digests [[buffer(1)]],
    ...
    uint in_base = gid * 8;
    for (uint i = 0; i < 8; i++) {
        state[i] = in_digests[in_base + i];
    }
    keccakf_1600(state);

    uint out_base = gid * 4;
    out_digests[out_base + 0] = state[0];
    out_digests[out_base + 1] = state[1];
    out_digests[out_base + 2] = state[2];
    out_digests[out_base + 3] = state[3];
}

// Two binary compress rounds in one dispatch: L → L+1 → L+2 (writes both new layers).
kernel void keccak_merkle_compress_two_levels(
    device const ulong* in_layer   [[buffer(0)]],
    device ulong*       mid_layer  [[buffer(1)]],
    device ulong*       out_layer  [[buffer(2)]],
    constant uint&      num_quads  [[buffer(3)]],
    uint gid [[thread_position_in_grid]]
) {
    if (gid >= num_quads) return;
    ...
    uint m1 = (2 * gid + 1) * 4;
    for (uint i = 0; i < 4; i++) {
        mid_layer[m0 + i] = p01[i];
        mid_layer[m1 + i] = p23[i];
    }
    ulong gp[4];
    keccak_compress_two_digests(p01, p23, gp);
    uint o = gid * 4;
    for (uint i = 0; i < 4; i++) out_layer[o + i] = gp[i];
}
...
Kb kb_from_canonical(uint x, Kb r_squared) {
    return kb_mul(Kb{x}, r_squared);
}

uint kb_to_canonical(Kb x) {
    uint t = x.v * KB_MONTY_MU;
    uint u_hi = mulhi(t, KB_P);
    return u_hi == 0u ? 0u : KB_P - u_hi;
}

kernel void poseidon2_pow_grind(
    constant uint*        base_state    [[buffer(0)]],
    constant Kb*          rc            [[buffer(1)]],
    ...
    if (nonce >= KB_P) return;

    Kb state[16];
    for (uint i = 0; i < 16; i++) state[i] = Kb{base_state[i]};

    Kb r2 = Kb{r_squared};
    state[witness_idx] = kb_from_canonical(nonce, r2);

    poseidon2_permute_16(state, rc);

    uint canonical = kb_to_canonical(state[7]);
...
kernel void kb_combine_select(
    ...
    for (uint i = 0; i < n; i++) {
        Kb r = Kb{right_table[b_right * n + i]};

        uint left_base = i * n_left + b_left_base;
        Kb l0 = Kb{left_table[left_base + 0]};
        Kb l1 = Kb{left_table[left_base + 1]};
        Kb l2 = Kb{left_table[left_base + 2]};
        Kb l3 = Kb{left_table[left_base + 3]};

        for (uint d = 0; d < 4; d++) {
            Kb ar = kb_mul(Kb{alphas[i * 4 + d]}, r);
            acc[d][0] = kb_add(acc[d][0], kb_mul(ar, l0));
            acc[d][1] = kb_add(acc[d][1], kb_mul(ar, l1));
            acc[d][2] = kb_add(acc[d][2], kb_mul(ar, l2));
            acc[d][3] = kb_add(acc[d][3], kb_mul(ar, l3));
        }
    }
